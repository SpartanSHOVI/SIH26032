...
import { centerApi, bookingApi, staffProcurementApi, procurementApi } from '../services/api';
import {
  getValidProcurementActions,
  saveCenterQueueCache,
  loadCenterQueueCache,
  applyOptimisticQueueStatus,
} from '../services/centerOfflineCache';
...
import { getCenterPaymentPolicy } from '../services/paymentPolicy';
...
export default function CenterDashboard() {
  ...
  const handleOpenSlip = async (token: any) => {
    const paymentPolicy = getCenterPaymentPolicy(token.booked_via);
    try {
      const res = await procurementApi.getReceipt(token.id);
      if (res?.data) {
        setSelectedSlipData({
          ...
          paymentMethod: paymentPolicy.defaultMethod,
          bankAccount: paymentPolicy.cashOnly ? 'Not required for cash payment' : res.data.bank_account,
          ifsc: paymentPolicy.cashOnly ? 'N/A' : res.data.ifsc,
          utrRef: paymentPolicy.cashOnly ? (token.transaction_ref || `CASH-${token.token_number}`) : res.data.utr_ref,
        });
    ...
    const weightKg = Number(token.quantity_received || token.quantity || 40) * 100;
    const rate = getMspRate(token.crop) || 2850;
    const amount = (weightKg / 100) * rate;

    setSelectedSlipData({
      organization: `कृ.उ.बा.स. ${selectedCenter?.name || 'नामपूर'}, उप बा करंजाड, नयाळाण, जि. नाशिक`,
      status: token.status === 'payment_completed' ? 'PAID' : token.status.toUpperCase(),
      sellerName: token.farmer_name || 'अविनाश खुशाल पाटील',
      sellerMobile: token.mobile || '9623037498',
      sellerVillage: token.address || 'अंतापूर',
      grainType: token.crop ? `${token.crop} (काडा उन्हाळी)` : 'काडा उन्हाळी (Kada Unhali)',
      receiptNo: `W28244-${new Date().toISOString().slice(2, 10).replace(/-/g, '')}-${token.token_number || '3'}`,
      lotNo: `W28244-${new Date().toISOString().slice(2, 10).replace(/-/g, '')}-${token.token_number || '3'}675-A`,
      bags: Math.ceil(weightKg / 50),
      weightKg: weightKg,
      ratePer100Kg: rate,
      totalAmount: token.payment_amount || amount,
      netAmount: token.payment_amount || amount,
      paymentMethod: paymentPolicy.defaultMethod,
      bankAccount: paymentPolicy.cashOnly ? 'Not required for cash payment' : (token.bank_account || 'Aadhaar Seeded DBT'),
      ifsc: paymentPolicy.cashOnly ? 'N/A' : token.ifsc,
      utrRef: token.transaction_ref || (paymentPolicy.cashOnly ? `CASH-${token.token_number}` : 'PFMS' + Date.now()),
    });
    setSlipModalOpen(true);
  };
  ...
  // Fetch Center Queue with offline caching and automatic local restore
    ...
    queryFn: async () => {
      ...
      try {
        if (typeof navigator !== 'undefined' && !navigator.onLine) {
          const cached = loadCenterQueueCache(selectedCenterId, selectedDate);
          if (cached && cached.length > 0) return cached;
        }
        const res = await centerApi.getQueue(selectedCenterId, selectedDate);
        const data = res.data || [];
        saveCenterQueueCache(selectedCenterId, selectedDate, data);
  ...
  // Reconnect synchronization listener
  useEffect(() => {
    const handleOnline = () => {
      setIsOffline(false);
      toast.success('Connectivity restored — synchronizing queue data');
      refetchQueue();
    };
    const handleOffline = () => {
      setIsOffline(true);
      toast.error('Offline mode: Using cached queue data');
    };
  ...
  // Helper for optimistic query updates and cache persistence
  const updateLocalQueue = (tokenId: number | string, newStatus: string, extraFields?: any) => {
    queryClient.setQueryData(['centerQueue', selectedCenterId, selectedDate], (old: any) => {
      if (!Array.isArray(old)) return old;
      const updated = applyOptimisticQueueStatus(old, tokenId, newStatus, extraFields);
      saveCenterQueueCache(selectedCenterId, selectedDate, updated);
      return updated;
    });
  };
  ...
  const callNextMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      const data = res.data;
      const farmerName = data.farmer_name && data.farmer_name !== 'undefined' ? data.farmer_name : 'Farmer';
      toast.success(`Called Token ${data.token_number} - ${farmerName}`);
      speakText(`Token number ${data.token_number}, ${farmerName}, please proceed to counter.`);
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: (_, vars) => {
      ...
      updateLocalQueue(vars.tokenId, vars.status, {
        reject_reason: vars.rejectReason,
        quantity_received: vars.quantityReceived,
      });
  ...
  // One-tap Gate Entry Mutation (Backend 1/4)
  const gateEntryMutation = useMutation({
    mutationFn: async ({ id }: { id: number | string; token_number?: string }) => {
      return await staffProcurementApi.gateEntry(id);
    },
    onSuccess: (_, vars) => {
      toast.success(`Token #${vars.token_number || vars.id} Gate Entry recorded!`);
      updateLocalQueue(vars.id, 'arrived');
      queryClient.invalidateQueries({ queryKey: ['centerQueue'] });
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.error || err.message || 'Gate entry failed');
    },
  });

  // One-tap Weighing Mutation (Backend 1/4)
  const weighingMutation = useMutation({
    mutationFn: async ({ id, quantity }: { id: number | string; token_number?: string; quantity?: number }) => {
      const net = quantity || 40;
      return await staffProcurementApi.weighing(id, {
        gross_weight: net + 12,
        tare_weight: 12,
        net_weight: net,
      });
    ...
    onSuccess: (_, vars) => {
      const net = vars.quantity || 40;
      toast.success(`Token #${vars.token_number || vars.id} Weighbridge recorded (${net} Qtl)!`);
      updateLocalQueue(vars.id, 'verification', { quantity_received: net });
      queryClient.invalidateQueries({ queryKey: ['centerQueue'] });
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.error || err.message || 'Weighing failed');
    },
  });

  // One-tap Quality Check Mutation (Backend 1/4)
  const qualityCheckMutation = useMutation({
    mutationFn: async ({ id }: { id: number | string; token_number?: string }) => {
      return await staffProcurementApi.qualityCheck(id, {
        moisture_percent: 12.0,
        quality_pass: true,
      });
    },
    onSuccess: (_, vars) => {
      toast.success(`Token #${vars.token_number || vars.id} Quality Check passed (Moisture 12%)!`);
      updateLocalQueue(vars.id, 'quality_check');
      queryClient.invalidateQueries({ queryKey: ['centerQueue'] });
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.error || err.message || 'Quality check failed');
    },
  });

  // One-tap Lot Accepted Mutation (Backend 1/4)
  const lotAcceptedMutation = useMutation({
    mutationFn: async ({ id, quantity }: { id: number | string; token_number?: string; quantity?: number }) => {
      return await staffProcurementApi.lotAccepted(id);
    },
    onSuccess: (_, vars) => {
      toast.success(`Token #${vars.token_number || vars.id} Lot Accepted into Mandi inventory!`);
      updateLocalQueue(vars.id, 'accepted', vars.quantity ? { quantity_received: vars.quantity } : undefined);
      setAcceptModalToken(null);
      queryClient.invalidateQueries({ queryKey: ['centerQueue'] });
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.error || err.message || 'Lot acceptance failed');
    },
  });

  // Reject Lot Mutation (Backend 1/4)
  const rejectLotMutation = useMutation({
    mutationFn: async ({ id, reason }: { id: number | string; reason: string }) => {
      return await staffProcurementApi.rejectLot(id, reason);
    },
    onSuccess: (_, vars) => {
      toast.success(`Token #${vars.id} marked Rejected`);
      updateLocalQueue(vars.id, 'rejected', { reject_reason: vars.reason });
      setRejectModalToken(null);
      queryClient.invalidateQueries({ queryKey: ['centerQueue'] });
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.error || err.message || 'Lot rejection failed');
    },
  });
  ...
  const updatePaymentMutation = useMutation({
    ...
    onSuccess: (data: any, variables: any) => {
      ...
      const paidToken = paymentModalToken;
      ...
      if (paidToken) {
        handleOpenSlip({
          ...
          payment_amount: variables.amount,
          transaction_ref: variables.ref,
        });
      }
    },
    onError: (err: any) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-primary-100 p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="bg-amber-50/90 border border-amber-200 rounded-lg px-3.5 py-2 flex items-center gap-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center justify-center gap-3 px-6 py-3 bg-gov-navy disabled:opacity-50 text-white font-bold rounded-lg shadow-md transition-all transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2"
        >
      ...
      {activeTab === 'queue' && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          ...
          {isOffline && (
            <div
              data-testid="center-offline-banner"
              className="m-4 p-3 bg-amber-50 border border-amber-300 text-amber-900 rounded-lg flex items-center justify-between text-sm"
            >
              <div className="flex items-center gap-2">
                <WifiOff className="w-4 h-4 text-amber-600 shrink-0" />
                <span>
                  <strong>Offline Mode Active:</strong> Displaying cached token queue from local storage. Actions will synchronize when network is restored.
                </span>
              </div>
              {lastSyncTime && (
                <span className="text-xs text-amber-700 opacity-80 whitespace-nowrap ml-2">Last synced: {lastSyncTime}</span>
              )}
            </div>
          )}
                  ...
                  filteredQueue.map((t: any) => (
                    <tr key={t.id} className="hover:bg-primary-50/40 transition-colors">
                      ...
                      <td className="px-4 py-3.5 whitespace-nowrap text-center">
                        {(() => {
                          const actions = getValidProcurementActions(t.status);
                          return (
                            <div className="flex items-center justify-center gap-1.5 flex-wrap">
                              {/* 1. Gate Entry */}
                              <button
                                data-testid={`action-gate-entry-${t.id}`}
                                disabled={!actions.canGateEntry || gateEntryMutation.isPending}
                                onClick={() => gateEntryMutation.mutate({ id: t.id, token_number: t.token_number })}
                                title={actions.gateEntryBlockedReason || 'Record Gate Entry (Mark Arrived)'}
                                className={`px-2.5 py-1 text-xs font-semibold rounded shadow-sm transition-all flex items-center gap-1 ${
                                  actions.canGateEntry
                                    ? 'bg-blue-600 hover:bg-blue-700 text-white cursor-pointer active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1'
                              ...
                              {/* 2. Weighing */}
                              <button
                                data-testid={`action-weighing-${t.id}`}
                                disabled={!actions.canWeigh || weighingMutation.isPending}
                                onClick={() => weighingMutation.mutate({ id: t.id, token_number: t.token_number, quantity: t.quantity || 40 })}
                                title={actions.weighingBlockedReason || 'Record Weighbridge Gross/Tare'}
                                className={`px-2.5 py-1 text-xs font-semibold rounded shadow-sm transition-all flex items-center gap-1 ${
                                  actions.canWeigh
                                    ? 'bg-indigo-600 hover:bg-indigo-700 text-white cursor-pointer active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-1'
                                    : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200 opacity-60'
                                }`}
                              >
                                Weighing
                              ...
                              {/* 3. Quality Check */}
                              <button
                                data-testid={`action-quality-check-${t.id}`}
                                disabled={!actions.canQualityCheck || qualityCheckMutation.isPending}
                                onClick={() => qualityCheckMutation.mutate({ id: t.id, token_number: t.token_number })}
                                title={actions.qcBlockedReason || 'Perform Moisture & Foreign Matter QC'}
                                className={`px-2.5 py-1 text-xs font-semibold rounded shadow-sm transition-all flex items-center gap-1 ${
                                  actions.canQualityCheck
                                    ? 'bg-purple-600 hover:bg-purple-700 text-white cursor-pointer active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-1'
                                    : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200 opacity-60'
                                }`}
                              >
                                QC Check
                              ...
                              {/* 4. Lot Accepted */}
                              <button
                                data-testid={`action-lot-accepted-${t.id}`}
                                disabled={!actions.canAccept || lotAcceptedMutation.isPending}
                                onClick={() => lotAcceptedMutation.mutate({ id: t.id, token_number: t.token_number, quantity: t.quantity || 40 })}
                                title={actions.acceptBlockedReason || 'Accept produce lot into Mandi inventory'}
                                className={`px-2.5 py-1 text-xs font-semibold rounded shadow-sm transition-all flex items-center gap-1 ${
                                  actions.canAccept
                                    ? 'bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-1'
                                    : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200 opacity-60'
                                }`}
                              >
                                <CheckCircle className="w-3 h-3" /> Lot Accepted
                              </button>

                              {/* 5. Reject Lot */}
                              {actions.canReject && (
                                <button
                                  data-testid={`action-reject-${t.id}`}
                                  onClick={() => setRejectModalToken(t)}
                                  title="Reject produce lot due to quality non-compliance"
                                  className="px-2 py-1 text-xs font-semibold bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200 rounded shadow-sm flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 focus-visible:ring-offset-1"
                                >
                              ...
                              {/* 6. Pay Farmer */}
                              {(t.status === 'accepted' || t.status === 'procured') && (
                                <button
                                  data-testid={`action-pay-${t.id}`}
                                  onClick={() => {
                                    const paymentPolicy = getCenterPaymentPolicy(t.booked_via);
                                    setPaymentModalToken(t);
                                    setPaymentMethod(paymentPolicy.defaultMethod);
                                    const qty = t.quantity_received || t.quantity || 0;
                                    setPaymentAmount(calculateMspPayment(t.crop, qty));
                                    setTransactionRef(
                                      paymentPolicy.cashOnly
                                        ? `CASH-${t.token_number}`
                                        : 'PFMS' + Math.floor(10000000 + Math.random() * 90000000),
                                    );
                                  }}
                                  className="px-2.5 py-1 text-xs font-semibold bg-green-600 hover:bg-green-700 text-white rounded shadow-sm flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-1"
                                >
                                  Pay Farmer
                                </button>
                              )}

                              {['payment_completed', 'procured', 'accepted'].includes(t.status) && (
                                <button
                                  onClick={() => handleOpenSlip(t)}
                                  className="px-2 py-1 text-xs font-bold bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded shadow-xs flex items-center gap-1 cursor-pointer"
                                  title="शेतकरी पावती (Print Farmer Receipt)"
                                >
                                  🧾 शेतकरी पावती
                                </button>
                              )}

                              {['payment_completed', 'rejected'].includes(t.status) && !['payment_completed'].includes(t.status) && (
                                <span className="text-xs text-gray-400 font-medium ml-1">Finished</span>
                              )}
                            </div>
                          );
                        })()}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  <optgroup label="⭐ Top Mandi MSP Commodities">
                    {POPULAR_MSP_CROPS.map((p) => (
                      <option key={p.name} value={p.name}>
                        {p.name} ({p.hindi}) · ₹{p.msp}
                      </option>
                    ))}
                  </optgroup>
                  {COMMODITY_GROUPS.map((grp) => (
                    <optgroup key={grp.name} label={`${grp.icon} ${grp.name} (${grp.count})`}>
                      {ALL_COMMODITIES.filter((c) => c.group_name === grp.name).map((c) => (
            ...
            <button
              ...
              className="w-full py-3 bg-primary-700 hover:bg-primary-800 disabled:opacity-50 text-white font-bold rounded-lg transition-colors shadow"
            >
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 max-w-2xl">
          ...
          >
            ...
            <button
              ...
              className="w-full py-3 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-bold rounded-lg transition-colors shadow flex items-center justify-center gap-2"
            >
      ...
      {activeTab === 'data_engineering' && (
        <div className="space-y-6">
          ...
          <div className="bg-gov-navy rounded-lg p-6 text-white shadow-sm border border-indigo-800/40 relative overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
              ...
              <div className="bg-slate-800/80 backdrop-blur-md rounded-lg p-4 border border-indigo-500/30 min-w-[280px]">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5 flex flex-col">
              ...
              <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between text-xs">
                ...
                <span className="text-gray-500 font-mono text-[11px]">
                  N = {(analytics?.moisture_distribution || []).reduce((acc: number, cur: any) => acc + (cur.count || 0), 0)} Assay Samples
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between pb-4 border-b border-gray-100">
              ...
              <span className="text-xs bg-purple-50 text-purple-700 font-semibold px-2.5 py-1 rounded-full border border-purple-100">
                Overall Efficiency: {analytics?.stage_funnel?.length ? `${analytics.stage_funnel[analytics.stage_funnel.length - 1]?.conversion_pct ?? 100}%` : '100%'}
              </span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-7 gap-3 mt-6">
              {(analytics?.stage_funnel || []).map((stage: any, idx: number) => (
                <div key={idx} className="relative group bg-slate-50 hover:bg-indigo-50/50 rounded-lg p-3.5 border border-gray-200 transition-all">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between pb-4 border-b border-gray-100">
              ...
              <span className="text-xs bg-amber-50 text-amber-700 font-mono font-bold px-2.5 py-1 rounded border border-amber-200">
                {(analytics?.engineered_features || []).length} Computed Features Live
              </span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
              {(analytics?.engineered_features || []).map((feat: any, idx: number) => (
                <div key={idx} className="bg-slate-50/80 rounded-lg p-4 border border-gray-200 hover:border-indigo-300 transition-all flex flex-col justify-between">
                  <div>
      ...
      {acceptModalToken && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-sm space-y-4">
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button
                ...
                onClick={() => {
                  if (acceptModalToken) {
                    lotAcceptedMutation.mutate({
                      id: acceptModalToken.id,
                      token_number: acceptModalToken.token_number,
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-sm space-y-4">
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button
                ...
                onClick={() => {
                  if (rejectModalToken) {
                    rejectLotMutation.mutate({
                      id: rejectModalToken.id,
                      reason: rejectReason,
                    });
                  }
                }}
      ...
      {paymentModalToken && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-sm space-y-4">
            ...
            {(() => {
              const paymentPolicy = getCenterPaymentPolicy(paymentModalToken.booked_via);
              return (
                <>
                  <p className="text-xs text-gray-600">
                    Farmer: <strong>{paymentModalToken.farmer_name}</strong> | Payment: <strong>{paymentPolicy.methodLabel}</strong>
                  </p>
                  {paymentPolicy.cashOnly && (
                    <div className="bg-amber-50 border border-amber-300 rounded-lg p-3 text-xs text-amber-900">
                      <strong>Assistant Booking:</strong> Bank details were not collected. This payout must be completed in cash at the mandi counter.
                    </div>
                  )}
                </>
              );
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  disabled={getCenterPaymentPolicy(paymentModalToken.booked_via).cashOnly}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white font-medium"
                >
                  {!getCenterPaymentPolicy(paymentModalToken.booked_via).cashOnly && (
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">
                {getCenterPaymentPolicy(paymentModalToken.booked_via).cashOnly ? 'Cash Receipt Reference' : 'Transaction Ref / PFMS UTR'}
              </label>
              <input
                type="text"
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-bold bg-green-600 hover:bg-green-700 text-white rounded-lg cursor-pointer"
              >
                Complete Payment & Print पावती
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Official APMC Shetkari Pavti Modal */}
