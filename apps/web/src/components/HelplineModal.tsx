...
export default function HelplineModal({ isOpen, onClose, defaultTab = 'ivr' }: HelplineModalProps) {
  ...
  const playVoicePrompt = (text: string, langCode: string) => {
    ...
    /**
     * Full BCP-47 speech code table — all 22 Indian scheduled languages.
     * Languages without a dedicated TTS voice fall back to their closest relative
     * so speech always fires and never silently skips.
     * Mirrors LANG_SPEECH_MAP in channel.controller.ts and SUPPORTED_LANGUAGES in i18n.ts.
     */
    const LANG_SPEECH_MAP: Record<string, string> = {
      ...
      'as':       'as-IN',
      'ne':       'ne-NP',
      // Fallbacks for languages without native browser TTS voice
      'brx':      'hi-IN',   // Bodo
      'doi':      'hi-IN',   // Dogri
      'ks':       'hi-IN',   // Kashmiri
      'mai':      'hi-IN',   // Maithili
      'mni-mtei': 'hi-IN',   // Manipuri
      'sa':       'hi-IN',   // Sanskrit
      'sat':      'hi-IN',   // Santali
      'sd':       'ur-IN',   // Sindhi → Urdu
      'kok':      'mr-IN',   // Konkani → Marathi
      'gom':      'mr-IN',   // Goan Konkani → Marathi
      // Accept BCP-47 codes passed directly from the API speech_code field
      'en-IN':    'en-IN',
      'hi-IN':    'hi-IN',
      'pa-IN':    'pa-IN',
      'mr-IN':    'mr-IN',
    ...
    // Resolve: prefer exact key match, else strip region suffix (e.g. 'pa-IN' → 'pa'), else Hindi
    const resolved =
      LANG_SPEECH_MAP[langCode] ??
      LANG_SPEECH_MAP[langCode?.split('-')[0]] ??
      'hi-IN';

    try {
      speakText(text, resolved);
    } catch (err) {
      console.warn('Speech synthesis unavailable or blocked:', err);
    }
    // Approximate duration based on text length and speaking rate
    setTimeout(() => {
      setIsSpeaking(false);
  ...
  const startIvrCall = async () => {
    ...
    try {
      ...
      const speechToPlay = data.tts_speech || speech;
      ...
      // Use speech_code (BCP-47) from API when available, otherwise use language key
      playVoicePrompt(speechToPlay, data.speech_code || data.language || ivrLang);
    } catch (e: any) {
